...
function index({
  ...
  //function useEffect getProductCommit <------------
  useEffect(() => {
    getProductCommit({
      id: id,
      page: 1,
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row items-start mt-4 justify-between">
        <div>
          <h1 className="text-[22px] md:text-[28px] text-sky-500 pb-3 font-bold">
            Izoh qoldirish
          </h1>
          <form onSubmit={(e)=>{hendelSubmit(e)}} className="mb-3" >
            <div className=" w-[400px] border broder-gray-300 rounded-md relative"> 
              <input
                type="text"
                name="message"
                placeholder="message..."
                className="w-full outline-none py-3 pl-3 pr-10 rounded-md"
              />
            <button type="submit" className=" absolute top-3 right-2">
              <SendIcon className="text-[#3a3af3]" fontSize="medium"/>
            </button>
            </div>
          </form>
        </div>
          ...
          {dataCommit.length ? (
            dataCommit.map((el) => {
              return (
                <div
                  key={el?.ID}
                  className="w-[500px] der-b rounded-lg pb-2 px-3 hover:shadow-md duration-300 cursor-pointer"
                >
                  <p className="felx items-center  text-gray-600 text-[18px]">
                    <span className="text-[#008524] pr-6">After:</span>{" "}
                    {el?.OwnerID}
                  </p>
                  <p className="felx items-center  text-gray-600 text-[18px]">
                    <span className="text-[#008524] pr-2">Masseg:</span>{" "}
                    {el?.Message}
                  </p>
                </div>
              );
